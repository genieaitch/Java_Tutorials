...
/*
가비지 컬렉션(Garbage CollectionEx)
메모리 관리 자동화를 위한 기능
프로그램 실행 중 더 이상 사용되지 않는 객체를 자동으로 찾아 제거함으로써 메모리 누수를 방지
개발자가 메모리를 명시적으로 해제할 피룡가 없도록 도와줌
...
//String의 경우 불변성으로 인해 적합한 예제는 아니지만 가장 직관적으로 가비지 컬렉션을 확인할 수 있어 예제 작성
    }
}
