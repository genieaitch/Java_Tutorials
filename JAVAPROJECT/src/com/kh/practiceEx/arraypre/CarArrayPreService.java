...
public class CarArrayPreService {
    public void carMethod1() {
        ...
        //todo : 2. cars 배열을 브랜드 이름 기준으로 오름차순 정렬
        //DB에서 ORDER BY ASC를 이용하면 됨!
            ...
            */
            ...
            compareTo 메서드 배열 내 존재하는 문자열 비교
            cars[j].brand       cars[j+1].brand     브랜드 이름을 비교했을 때 가나다 순으로 봤을 때 결과가 0보다 크다면 값 위치를 교체
            */
        ...
        for (int i = 0; i < cars.length -1; i++) {//
            for (int j = 0; j < cars.length; j++) {
                if (cars[j].brand.compareTo(cars[j + 1].brand) > 0) {
                    ...
                    cars[j] = cars[j + 1]; //비교했을 때 가나다 순으로 앞에 있어야 하는 단어 위치를 앞으로 변경
...
/*
        ...
        boolean carEquals = Arrays.equals(carsEquals,cars);
        System.out.println("cars와 carsEquals 안에 들어있는 차들은 같은가? : "+carEquals);
    }
