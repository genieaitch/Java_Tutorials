...
2차원 배열
[첫 번째 시작하는 {} 배열 갯수] [{{배열 안에 몇 개의 값을 배치할 지 설정}}]
int[][] arr = {{},{},{}};
...
public class ArrayPre {
    ...
    public void maxValue() {
        ...
        /*for(int i=1 이 0이 아니라 1로 시작하는 이유
          ...
          첫번 째 자리값은 이미 최대값 후보자리 선정
          첫 번째 자리값과 두 번째 자리 값을 비교 진행해서
          두 번째 자리값이 첫 번째 자리값보다 크다면 두 번째 자리 값을 첫 번째 자리 값에 넣어줌
        */
        for (int i = 1; i < numbers.length; i++) {
            if (max < numbers[i]) { //만약에 비교할 숫자가 max에 들어있는 최대값보다 크다면
                max = numbers[i]; //최대 값을 현재 비교하고 있는 숫자로 교체하기
            }
    ...
    public void findString() {
        ...
        String targer = "cherry";

        // TODO : 특정 문자의 위치를 찾고 출력하는 for문 완성

        /*위치 관계없이 단어 찾기 for문
        for(String word : words) {
            if(word.equals(targer)) {
        ...
        for (int i = 0; i < words.length; i++) {//특정 위치 찾기 for문
            if (words[i].equals(targer)) {
                System.out.println("찾는 문자열 [" + targer + "] 은 배열의 " + i + "번째 위치해 있습니다.");
                break;
            }
        }
    }
    ...
    public void replaceArrayNumber() {
        ...
        //for문으로 배열을 순회해서 만약에 교체할 숫자랑 배열에 들어있는 숫자가 일치하다면 99로 교체
    ...
    public void find2DArray() {
            ...
            for (int i = 0; i < words.length; i++) {
                for (int j = 0; j < words[i].length; j++) {
                    if (words[i][j].equals(target)) {
                        System.out.println(target + "은 [" + (i) + "][" + (j) + "] 에 위치에 존재합니다");
                    }
                }
            }
        }
        ...
        public void replaceStringIn2DArray() {
            String[][] colors = {{"red", "green", "blue"}, {"yellow", "blue", "purple"}, {"pink", "blue", "orange"}};
            String target = "blue";
            String replacement = "cyan";

            for (int i = 0; i < colors.length; i++) {//값을 꺼내기 1단계
                for (int j = 0; j < colors[i].length; j++) {//1단계에서 찾을 값을 꺼내기
                    if (colors[i][j].equals(target)) {//만약에 target과 replacement를 비교했음
                        colors[i][j] = replacement;//blue가 존재하는 위치에 cyan 넣어주기
                    }
                }
                System.out.println();// 각 행 출력 후 줄바꿈
            }

            /*System.out.println(Arrays.toStrign(colors));
            System.out.println(Arrays.deepToString(colors));
            System.out.println("===blue → cyan 변경 완료===");*/

            //결과 배열 출력
            for(int i = 0; i < colors.length; i++){//{맨 처음에 감싼 배열 순회}
                for(int j = 0; j < colors[i].length; j++){//{두 번째로 감싸져 있는 배열}
                    System.out.print(colors[i][j] + " ");//세로 0번 째 부터 가로 0,1,2 순회
                }
                System.out.println();// 각 행 출력 후 줄바꿈
            }
        }
        ...
        public void lunchMenu2DArray() {
            ...
            //toString은 1차원 배열만 값을 출력하므로 각 배열 내 행 주소 값 출력
            System.out.println(Arrays.toString(lunchMenu));

            //deepToString은 2차원 이사으이 배열 내 존재하는 값을 출력
