...
2. java.nio 패키지(폴더)
    New IO의 약자로, 더 효율적이고 비동기적인 데이터 처리를 지원
...
同 = 같을 동
期 = 기약할 기
동기적으로 실행되는 대화 = 말을 시작한 사람이 말을 끝내야 다른 사람이 말을 끝낼 수 있음
...
호출한 쪽은 결과를 기다려야하므로 작업이 순차적으로 실행
...
비동기적으로 실행되는 대화 : 여러 사람이 동시에 서로 다른 이야기를 하며 대화가 겹치는 상황
작업을 요청한 것과 별개로 다른 작업이 진행되는 방식

작업이 완료되면 알림을 통해 결과를 전달 받음
*알림 : 콜백 이벤트
호출한 쪽은 작업이 끝날 때까지 기다리지 않아도 되므로 병렬 처리 가능
효율적이지만 구조가 복잡해질 수 있음
예) 음식점에서 음식을 주문한 뒤, 음식을 기다리지 않고 대화를 하거나 다른 일을 하다가 음식이 나와서 가져오는 것
*/
